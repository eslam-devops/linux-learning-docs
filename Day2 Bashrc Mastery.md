...
تمام ✅
اللي واضح من المخرجات اللي عرضتها 👇

```
╭─(root＠NTI-SERVER)──[]
```

إن المتغير `LAST_CMD` مش بيظهر جوه الـ PS1، وده طبيعي يحصل لو:

1. `PROMPT_COMMAND` مش متعرف أو متشغلش صح.
2. أو إنك كتبت `$LAST_CMD` من غير ما تحط الـ escape صح داخل الـ PS1.
3. أو إنك محتاج تعمل `source ~/.bashrc` بعد ما تعدّل.

🧰 علشان نحل المشكلة ونخلي آخر أمر يكتب فعلاً جوه prompt:
...
#### 2. أضف السطر ده (لو مش موجود):

```bash
PROMPT_COMMAND='LAST_CMD=$(history 1 | sed "s/^[ ]*[0-9]\+[ ]*//")'
```

📌 لو عايز تقصر الأمر لو طويل جدًا (اختياري):

```bash
PROMPT_COMMAND='LAST_CMD=$(history 1 | sed "s/^[ ]*[0-9]\+[ ]*//" | cut -c1-40)'
```

---

#### 3. عدّل الـ PS1 كالتالي 👇 (لاحظ الـ `\${LAST_CMD}` مهم جدًا):

```bash
export PS1="\[\e[38;5;208m\]╭─(\[\e[38;5;220m\]\u\[\e[38;5;208m\]＠\[\e[38;5;118m\]\h\[\e[38;5;208m\])──[\[\e[38;5;226m\]\${LAST_CMD}\[\e[38;5;208m\]]\n\[\e[38;5;208m\]╰─\[\e[38;5;118m\]\w\[\e[0m\] 🐧 "
```

⚠️ لاحظ:

* استخدم `\${LAST_CMD}` (مش `$LAST_CMD`) علشان يتفسر عند عرض الـ prompt مش وقت تعريفه.
* استخدم علامات escape `\` صح علشان تحافظ على الألوان.
...
```
╭─(root＠NTI-SERVER)──[vim .bashrc]
╰─~ 🐧
```

---

لو عايز كمان أضيف لون مختلف لاسم الأمر (مثلاً أزرق أو أخضر) أقدر أظبطهولك 🎨
تحب أعطيك نسخة ملونة لاسم الأمر؟ 🟢🔵✨
